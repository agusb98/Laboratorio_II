...
namespace Clase06
{
    public class Paleta
    {
        ...
        public static Paleta operator -(Paleta paletaUno, Tempera temperaUno)
        {
            ...
            //if (paletaUno == temperaUno)//Si coindicen, se borra
            //{
            //paletaUno._temperas[paletaUno.ObtenerIndice()] = n; //De la paleta pasada por prametro, llamamos a la funcion obtener indice para que nos devuelva un indice con null y lo rellenemos con la tempera pasada como parametro

            //}
            int i = 0;
            foreach (Tempera t in paletaUno._temperas)
            {

                if(t == temperaUno)
                {
                    paletaUno._temperas[i] = null;
                    break;
                }
                i++;
            }
