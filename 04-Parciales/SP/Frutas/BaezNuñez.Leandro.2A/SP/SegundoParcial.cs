...
namespace SP
{
    ...
    public partial class SegundoParcial : Form
    {
        ...
        {
            MessageBox.Show("Baez Nuñez Leandro 2A");
        }
        ...
        {
            SqlCommand comando = new SqlCommand("SELECT * FROM frutas");
            SqlConnection conexion = new SqlConnection(Properties.Settings.Default.Conexion);
            ...
            while (lector.Read())
            {
                sb.AppendLine(string.Format("Id: {0} - Nombre: {1} - Peso: {2} - Precio {3}", i, lector["nombre"].ToString(), lector["peso"].ToString(), lector["precio"].ToString()));
                i++;
            }
