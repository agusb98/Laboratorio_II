    ...
    public partial class SegundoParcial: Form
    {
        ...
        private static string ObtenerListadoFrutas() {
            ...
            try {
                ...
                string consulta = string.Format("SELECT * FROM frutas");
                ...
                while (sqlDataReader.Read()) {
                    string tipoFruta = sqlDataReader["nombre"].ToString();
                    double auxPeso = Convert.ToInt64(sqlDataReader["peso"]);
                    double auxPrecio = Convert.ToInt64(sqlDataReader["precio"]);

                    sb.AppendFormat("Fruta: {0} - Peso: {1} - Precio {2}", tipoFruta, auxPeso, auxPrecio);
                    sb.AppendLine();
                }
                conexion.Close();
                return sb.ToString();
